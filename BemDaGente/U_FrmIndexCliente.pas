...
begin
  ...
  try
    ...
    AddToInitProc(swalSuccess('CONFIRMAÇÃO', 'Cadastro do Chamado realizado com sucesso !'));
    Controller.DM.FDQChamadosCad.Close;
    MontaChamadosAbertos;
  except
    AddToInitProc(swalAlert('ATENÇÃO','Ocorreu um erro na gravação do Chamado tente novamente !'));
  end;
end;
...
begin
  ...
  begin
    AddToInitProc(swalError('Atenção', 'Senha Atual Inválida !!!'));
    Exit;
  end;
  Controller.DM.FDQQuery1.Close;
  ...
  begin
    AddToInitProc(swalError('Atenção', 'Novas Senhas não conferem, tente novamente...'));
    Refresh;
    Exit;
  end;
  ...
  AddToInitProc(swalSuccess('Confirmação', 'Senha trocada com sucesso !!!'));
end;
